...
for workbookPath in workbookPaths:
    ...
    for sheet in wb._sheets:
        # Get init File Path
        filePath = sheet.cell(row=1, column=col).value
        # Backup Original Files
        addFilePaths(filePath)
        id = 2
        while sheet.cell(row=id, column=col).value != 'end':
            if sheet.cell(row=id, column=col).value != None:
                if sheet.cell(row=id, column=col).value != 'end' and not':' in sheet.cell(row=id, column=col).value:
                    filePath = sheet.cell(row=id, column=col).value
                    addFilePaths(filePath)
            id += 1
